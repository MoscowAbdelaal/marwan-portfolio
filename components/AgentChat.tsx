...
export default function AgentChat() {
  ...
  return (
    <section id="agent" className="py-16 sm:py-20 md:py-24 px-4 sm:px-6 bg-zinc-900/40 animate-fade-in">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-950 overflow-hidden">
          <div className="h-72 sm:h-[420px] overflow-y-auto p-4 sm:p-6 space-y-3 sm:space-y-4">
            {messages.map((msg, i) => (
              ...
              >
                <div
                  className={`max-w-[85%] sm:max-w-[80%] rounded-2xl px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm leading-relaxed ${
                    msg.role === "user"
          ...
          <div className="px-4 sm:px-6 pb-2 sm:pb-3 flex flex-wrap gap-1.5 sm:gap-2">
            {suggestions.map((s) => (
              <button
                key={s}
                onClick={() => setInput(s)}
          ...
          <form onSubmit={sendMessage} className="p-3 sm:p-4 border-t border-zinc-800 flex gap-2 sm:gap-3">
            <input
              ...
              className="flex-1 bg-zinc-900 border border-zinc-800 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-zinc-600"
            />
            <button
              type="submit"
              disabled={loading || !input.trim()}
              className="px-4 sm:px-5 py-2 sm:py-3 rounded-xl bg-zinc-100 text-zinc-900 text-xs sm:text-sm font-medium hover:bg-white disabled:opacity-40 transition"
            >
              Send
            </button>
          </form>
        ...
        <p className="text-[10px] sm:text-xs text-zinc-600 mt-3 sm:mt-4 text-center">
